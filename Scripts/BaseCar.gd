...
func _ready():
	...
	# Random visibility setup per group
	for i in range(1,7):
		var group_path = "player/Group " + str(i)  # Note the space after "Group"
		print("Looking for:", group_path)
		if has_node(group_path):
			...
			if children.size() >= 1:
				...
				print("Group %d doesn't have enough children!" % i)
		else:
			print("Group %d not found at path: %s" % [i, group_path])
...
func _on_area_3d_body_entered(body: Node3D) -> void:
	...
	elif body.is_in_group("StreetLight"):
		get_tree().change_scene_to_file("res://Scenes/bad_light.tscn")  # Load the "Bad Stop" scene 
		reset_score()	
...
func apply_street_light_penalty():
	get_tree().change_scene_to_file("res://Scenes/bad_light.tscn")  # Load the "Bad Stop" scene
	reset_score()
